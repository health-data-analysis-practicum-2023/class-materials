...
rename$drink <- ifelse(dat$ALQ_1 == 0, 4, dat$ALQ_1_UNIT)


hy_df <- rename %>% mutate(gender = factor(gender, levels=c(1,2),
                                           labels=c('Male', 'Female')),
                           income = factor(income, levels=c(1:6),
                                           labels=c('Less than $20,000', '$20,000 - $39,999',
                                                    '$40,000 - $59,999', '$60,000 - $79,999',
                                                    '$80,000 - $99,999', '$100,000 or more')),
                           drink = factor(drink, levels=c(4:1), ### Reorder drink variable
                                          labels=c('Never', 'Yearly', 'Monthly', 'Weekly')),
                           smoking = factor(smoking, levels=c(3:1), ### Reorder smoking variable
                                            labels=c('Never smoker', 'Former smoker', 'Current smoker')),
                           hypertension = factor(hypertension, levels=c(2,1), ### Reorder hypertension variable
                                                 labels=c('No', 'Yes'))
                           ) 
...
```{r}
hy_p_df %>%
  ggplot(aes(x = hypertension, fill = drink, position = "dodge")) + 
  geom_bar(position = "fill") + 
  facet_wrap(~gender) +
  labs(title = "Distribution of alcohol consumption among those with and without hypertension",
       subtitle = "stratified by gender",
       y = "proportion",
       fill = "Frequency of alcohol consumption") + 
  theme(plot.title = element_text(size = 13)) 

```

Overall, there are greater proportion of weekly alcohol consumption among those without hypertension for both male and female. The yearly alcohol consumption seems to be similar in both groups with and without hypertension for each male and female strata. The proportion for monthly alcohol consumption is greater in those without hypertension for both male and female. The proportion of those who never drink alcohol is higher among those with hypertension for both male and female.
...
```{r}
sexdb <- hy_df %>% 
  filter(!is.na(diabetes) == TRUE) %>% 
  count(diabetes, gender, hypertension) %>% 
...
sexdb %>%
  ggplot() + 
  geom_bar(aes(x = diabetes, y = prop), stat = "identity", color = "black", fill = "lightcoral") +
  geom_text(aes(x = diabetes, y = prop, label = prop), vjust = -0.25) +
  labs(title = "Comparing the Risk of Hypertension by Diabetes Status",
    subtitle = "Stratified by Gender",
      x = "Diabetes Status",
      y = "Percentage",
      caption = "Figure 1: Risk of hypertension by diabetes status and stratified by gender"
      ) + 
  facet_wrap(~gender) +
...
As see in Figure 1, the proportion of individuals with hypertension increases as diabetes status moves from no diabetes to prediabates to diabetes. This holds true for both males and females. The largest differences in hypertension prevalence appear to be between no diabetes and diabetes for both genders with an increase from 23.7 to 67.6 percent for males and 20.8 to 68.9 percent for females.   
...
Each panel in the display suggests whether race, diabetes, and cholesterol, respectively, is associated with having hypertension. From the first panel, we see that a larger proportion of Black and African American as well as Pacific Islander respondents have a higher risk of having hypertension than White, American Indian/Alaska Native, Asian, and respondents of another race not listed. Therefore, it is reasonable to conclude from this data visualization that race is associated with having hypertension. In the second panel, the risk of hypertension is highest among those who have diabetes and lowest among those who do not have diabetes. Therefore, it is reasonable to conclude from this data visualization that diabetes in associated with having hypertension as well. In the third panel, the risk of hypertension is higher among those with a high cholesterol and lower among those with a low cholesterol. Therefore, it also is reasonable to conclude from this data visualization that cholesterol is also associated with having hypertension. 
...
rename <- cleandat %>%
  mutate(educate = factor(educate, levels=c(4,3,2,1), labels=c("College Grad",
        ...
        insurance = factor(insurance, levels=c(1, 2), labels = c("With Insurance", "No Insurance")),
...
plot3<-rename %>%
  ggplot(aes(x=race, fill= hypertension))+
    geom_bar(position="fill")+
    labs(title = "Distribution of Race by Hypertension",
         subtitle = "Blue color for With Hypertension")+
    theme_bw()+
...
plot4<-rename %>%
  ggplot(aes(x=insurance, fill= hypertension))+
    geom_bar(position="fill")+
    labs(title = "Distribution of Insurance by Hypertension",
         subtitle="Blue color for With Hypertension")+
    theme_bw()+
...
From the graphs, we are able to observe that bmi,educational levels, insurance, race, and diabetes are associated with having hypertensions. 
Specifically, 
* Pacific islander and black/african american have a higher chance of getting hypertension than the other race groups. 
*  individuals with insurance have a higher chance of getting hypertension than individuals without insurance. 
...
### include never drinkers in drink variable
rename <- rename %>% 
          mutate(drink = ifelse(dat$ALQ_1 == 0, 4, drink))
...
p2b <- hy_p_df %>%
  ...
  ggtitle("Distribution of hypertension by race") +
  ylab('proportion')
ggarrange(p2a, p2b, ncol = 1, nrow = 2)
```

The display shows the counts and proportions of hypertensive patients among race groups. While the proportion graph indicates that the Black and Pacific Islander populations are at increased risk of disease, the count graph shows that the sample population of the Pacific Islander and Indian/Native Alaskan are very small and thus the high proportion of hypertensive patients in the sample population cannot be generalized to the race group. Nevertheless, Blacks have a significantly higher proportion of hypertension with a large sample population - indicating that race is associated with hypertension. This association may manifest through genetic, social and behavioral, or environmental mediums. 
...
# combine the two alcohol variables into one
rename <- rename %>% 
          mutate(drink = ifelse(dat$ALQ_1 == 0, 4, drink))
...
# table of hypertension proportions for different smoking statuses 
my_table3 <- hy_p_df %>%
  count(hypertension, smoking) %>%
  group_by(smoking) %>%
  mutate(proportion =round(n/sum(n),2))

# data display of distribution of hypertension among different smoking statuses 
my_table3 %>%
  filter(hypertension=="Yes") %>%
  ggplot() + 
  geom_bar(aes(x = smoking, y = proportion, fill = hypertension), 
           stat = "identity", position = "dodge") +
  theme(legend.position="none", axis.text.x = element_text(angle = 45, hjust=1)) + 
  geom_text(aes(x = smoking, y = proportion, label = proportion), vjust = -.25) +
  labs(title = "distribution of hypertension by smoking status") + 
  ylab("proportion of hypertension")

```

Comparing the distribution of hypertension by smoking status, a higher proportion of never smokers (0.34) than former and current smokers have hypertension. 



<details> <summary> A few thoughts for when we return from our groups </summary>

### About that smoking variable...

 There was a mistake in my code from a previous lecture for how I recoded the `SMOKER3CAT` variable. It has been corrected below to match what is in the [Variable Codebook](https://med.nyu.edu/departments-institutes/population-health/divisions-sections-centers/epidemiology/sites/default/files/nyc-hanes-datasets-and-resources-public-dataset-codebook.pdf){target="_blank"}. Yi or I will also include a modification to the code in your assignments when we grade them this week, although not many of you are currently using this variable for your visualizations. But if you do use it in the future, you'll want to make sure it is coded correctly!
...
* Submit your data display and the code for your initial survey-weighted analysis in R Markdown through Github by Sunday (March 5, 2023) at midnight.
