...
The code below includes recoding of the variables, including creating the new `drink` variable with levels in the order discussed.
...
dat %>% mutate(new_drink = case_when(ALQ_1 == 0 ~ 4, 
                                       ALQ_1_UNIT == 1 ~ 1,
                                       ALQ_1_UNIT == 2 ~ 2,
                                       ALQ_1_UNIT == 3 ~ 3)) %>% count(new_drink)

# bad coding practice!
#dat %>% mutate(new_drink_2 = if_else(ALQ_1 == 0 , 4, ALQ_1_UNIT , missing = 99) )%>% count(new_drink_2)
...
Notice that we ordered the `drink` variable as `Never`, `Yearly`, `Monthly` and `Weekly`,  in increasing order of frequency.
...
From this plot, we see that we need to pay attention to the missing values here, since we are getting a boxplot for the missing values.  We can instead use our data set with the observations with missing values removed, `hy_p_df`.
...
Comparing the medians of these two boxplots, we see that people who are hypertense tend to be older than people who are not (median age for non-hypertense individuals `r median(hy_p_df %>% filter(hypertension == "No") %>% pull(age))`, median age for hypertense individuals `r median(hy_p_df %>% filter(hypertension == "Yes") %>% pull(age))`), which indicates that age is related to hypertension.
...
<details><summary> Click here for details on how to improve the data display of the relationship between gender and hypertension </summary>
...
This is a good lesson to keep in mind as you create your own data visualizations this week for your assignment.
</details>
