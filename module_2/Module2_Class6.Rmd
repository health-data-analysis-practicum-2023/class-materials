...
### A few comments from your submissions from this week


* Remember to always double check that the document can knit before you submit it, if at all possible. 

* Most of you did a good job ensuring that the correct weights were used. In almost all cases, the BMI variable is the reason why you need to use EXAM_WT instead of CAPI_WT, so if you don't include BMI in your model, you should very likely use CAPI_WT.

#### Example of formatted tables from Piazza post
...
race_table <- tidy(htn_model, exponentiate = TRUE, conf.int = TRUE)%>%
  ...
  mutate("95% confidence interval" = paste0("(", round(conf.low, digits = 3), ", ", round(conf.high, digits = 3), ")")) %>%
  ...
  kable(format = "html", align = 'r', caption = "Weighted logistic regression model examining the effect of race (compared to non-Hispanic white) on risk of hypertension, controlled for diet, activity, age and BMI")
...
diet_table <- tidy(htn_model, exponentiate = TRUE, conf.int = TRUE)%>%
  ...
  kable(format = "html", align = 'r', caption = "Weighted logistic regression model examining the effect of diet quality (compared to a poor diet) on risk of hypertension, controlled for race, activity, age and BMI")
...
other_table <- tidy(htn_model, exponentiate = TRUE, conf.int = TRUE)%>%
  ...
  kable(format = "html", align = 'r', caption = "Weighted logistic regression model examining the effect of exposures on risk of hypertension, controlled for race and diet")
```


```{r}
race_table %>%
  kable_minimal(full_width = F, position = "left", c("striped", "hover"))
other_table %>%
...
* Submit your data display(s) and the code for your updated survey-weighted analysis in R Markdown through Github by Sunday March 12, 2023 at midnight.
...
* Start with a *general* area of public health in which you have interest; then narrow to a *specific* question you'd like to answer
* It can be helpful to frame your question in terms of investigating a relationship between a specific outcome variable (like "disease status" from our Module 1) and one or more primary predictor variables ("smoking status" from our Module 1).
