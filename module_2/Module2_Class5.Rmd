...
```{r}
hy_p_df %>% count(smoking, hypertension) %>% group_by(smoking) %>% mutate(prop = n/sum(n)) %>% filter(hypertension == "Yes") %>% 
  ggplot() +
...
hy_p_df %>% ggplot() + geom_boxplot(mapping = aes(x = hypertension, y = age))
...
Here is link to the [doc](https://docs.google.com/document/d/1R-8157ozdOPi-09h3EsconPV0GK9ZhxuY6hRQ5QiMpM/edit?usp=sharing){target="_blank"}.
