...
#data visualizations comparing the results separating categorical and continuous variables 
g1_both %>% 
  ggplot(aes(x = odds.ratio, y = fct_reorder(term, odds.ratio)), group_by(method)) +
     geom_vline(xintercept = 1, linetype = "dashed") +
     geom_errorbarh(aes(y=fct_reorder(term, odds.ratio), xmin = conf.low, 
     xmax = conf.high, color = method), linewidth = 0.8, position="dodge")+
     geom_point(aes(color = method,
                    shape = method), size = 3, position=position_dodge(width=0.9))+
  labs(title = "hypertension weighted vs unweighted logistic regression", 
...
g1_both %>%
  ggplot(aes(x = odds.ratio, y = fct_reorder(term, odds.ratio), group_by(method), color = method)) +
  ...
  geom_point(aes(color = method, shape = method), size = 3, position=position_dodge(width=0.9)) +
  scale_color_manual(values = c("#0072B2", "#f08080")) +
  scale_shape_manual(values = c(19, 17)) +
  theme_minimal() +
  labs(title = "Comparison of Survey-Weighted and\nNon-Survey Weighted Regression Analysis",
       y = "Variable",
