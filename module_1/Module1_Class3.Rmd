...
nmes_data <- nmes_data %>%
  mutate(disease = factor(lc5 == 1 | chd5 == 1, levels=c(FALSE, TRUE),
...
If you want your table to include the combinations of levels that have no people in them, you want to include the argument `.drop = FALSE` to the call to `count`:

```{r}

nmes_data %>%
  count(eversmk, disease, educate, .drop=FALSE) %>%
